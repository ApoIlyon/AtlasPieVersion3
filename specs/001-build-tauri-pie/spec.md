...
- **Profile**: Набор параметров pie-меню (название, хоткей, активность, связанный набор `PieMenu`).
- **PieMenu**: Конфигурация кругового меню, включающая массив сегментов, визуальные параметры, глубину вложенности.
- **PieSlice**: Сегмент меню с текстом, иконкой, хоткеем, ссылкой на `Action` и возможным дочерним `PieMenu`.
- **Action**: Абстракция выполняемых действий (тип, параметры запуска, макрокоманды, задержки).

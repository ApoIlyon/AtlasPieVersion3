...
- **FR-004**: Система ДОЛЖНА поддерживать структуру pie-меню до N сегментов (минимум 2, максимум 12) с вложенными уровнями.
...
- **FR-009**: Система ДОЛЖНА обеспечивать автозапуск при входе в Windows с управлением через UI (включить/отключить).
...
- **FR-011**: Интерфейс ДОЛЖЕН следовать тёмной теме и стилю, вдохновлённому `kando-2.0.0`, включая анимации и композицию кругового меню.
...
- **FR-017**: Система ДОЛЖНА проверять валидность макросов и предоставлять визуальный редактор/ввод с подсветкой ошибок перед сохранением.
- **FR-018**: Приложение ДОЛЖНО работать офлайн и корректно обрабатывать отсутствие сетевого доступа.
- **FR-019**: Система ДОЛЖНА иметь механизм резервного копирования и восстановления пользовательских данных при обновлениях.
...
- **FR-021**: Система ДОЛЖНА обрабатывать автозапуск и фоновые службы через механизмы Windows Task Scheduler/Registry, macOS LaunchAgents и Linux systemd/xdg autostart.
- **FR-022**: Система ДОЛЖНА предоставлять унифицированный UX при уважении платформенных паттернов (иконки меню, сочетания клавиш, жесты), включая адаптацию под macOS меню в строке меню и Linux tray environment fallback.
...
- **FR-025**: Система ДОЛЖНА периодически проверять GitHub-релизы проекта (аналогично `kando-2.0.0`), уведомлять пользователя о доступности новой версии и предоставлять ссылку/кнопку для ручного скачивания установщика.
...
- **NFR-001**: Среднее время появления pie-меню после нажатия хоткея не превышает 50 мс на поддерживаемых платформах.
- **NFR-002**: Время запуска действия (от выбора сегмента до подтверждения) не превышает 200 мс в 95-м перцентиле на тестовых профилях.
- **NFR-003**: FPS в UI-оболочке не опускается ниже 60 кадров при активном предпросмотре и трёх открытых профилях.
- **NFR-004**: Память процесса `autohotpie-tauri` не превышает 150 МБ при загрузке трёх профилей и журналов действий.

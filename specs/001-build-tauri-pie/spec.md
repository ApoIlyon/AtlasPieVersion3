...
- **NFR-006**: Поддерживаемые платформы (Windows 10/11, macOS 13+, Linux GTK/KDE) получают платформенно-специфичную интеграцию (трей, меню, автозапуск) при следующих измеримых критериях паритета UX:
  ...
  4. **Linux fallback**: При отсутствии системного трея появляется Linux fallback-панель с тем же набором действий и локализаций; Playwright-сценарий `linux-ux.spec.ts` подтверждает сценарии "Toggle Autostart", "Switch Profile", "Open Logs".

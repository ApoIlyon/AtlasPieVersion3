...
export class PointerInput extends InputMethod {
  ...
  /** If greater than zero, right-clicks before this timestamp will be ignored. */
  private ignoreCloseBefore = 0;
  ...
  /** Ignore the next right-click which would usually close the menu. */
  public ignoreNextCloseEvent() {
    this.ignoreCloseBefore = Date.now() + 100;
  }
  ...
  public onPointerDownEvent(event: MouseEvent | TouchEvent) {
    ...
    // Go back or hide the menu on right click events. Ignore right-clicks immediately
    // after showing the menu.
    if ((event as MouseEvent).button === 2) {
      if (Date.now() < this.ignoreCloseBefore) {
        this.ignoreCloseBefore = 0;
      } else {
        this.closeCallback();
      }
      return;

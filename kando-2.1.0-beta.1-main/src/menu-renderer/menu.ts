...
export class Menu extends EventEmitter {
  ...
  /** Holds the keys of the shortcut which opened the menu. */
  private triggerKeys: Set<string> = new Set();

  /** Listener used to close the menu when the trigger keys are released. */
  private onTriggerKeyUp = (ev: KeyboardEvent) => {
    const key = this.isModifier(ev.key) ? ev.key.toLowerCase() : ev.code.toLowerCase();
    if (this.triggerKeys.has(key)) {
      this.triggerKeys.delete(key);
      if (this.triggerKeys.size === 0 && !this.container.classList.contains('selected')) {
        this.cancel();
      }
    }
  };

  /** Listener used to close the menu when the trigger mouse buttons are released. */
  private onTriggerMouseUp = (ev: MouseEvent) => {
    const names = [
      'mouseleft',
      'mousemiddle',
      'mouseright',
    ...
    const key = names[ev.button];
    if (key && this.triggerKeys.has(key)) {
      this.triggerKeys.delete(key);
      if (this.triggerKeys.size === 0 && !this.container.classList.contains('selected')) {
        this.cancel();
      }
  ...
  /** Checks whether the given key represents a modifier key. */
  private isModifier(key: string) {
  ...
  public show(root: RenderedMenuItem, showMenuOptions: ShowMenuOptions) {
    ...
    document.removeEventListener('keyup', this.onTriggerKeyUp);
    document.removeEventListener('mouseup', this.onTriggerMouseUp);
    this.triggerKeys.clear();
    if (showMenuOptions.trigger) {
      this.triggerKeys = this.parseTrigger(showMenuOptions.trigger);
      if (this.triggerKeys.size > 0) {
        document.addEventListener('keyup', this.onTriggerKeyUp);
        document.addEventListener('mouseup', this.onTriggerMouseUp);
      }
    }
    ...
    if (turboEnabled && !showMenuOptions.hoverMode) {
      if (this.triggerKeys.size > 0) {
        // When the menu stays open until all trigger keys are released, we must
        // prevent turbo mode from activating due to these keys being held.
    ...
    this.pointerInput.ignoreNextCloseEvent();
    ...
    this.pointerInput.enableTurboMode = turboEnabled;
  ...
  /** Resets the menu to its initial state without hiding it. */
  public reset() {
    if (!this.root || !this.showMenuOptions) {
      return;
    }
  ...
  /**
   * Parses an accelerator string and returns the keys that should be monitored for
   * release events.
   */
  private parseTrigger(trigger?: string): Set<string> {
    ...
    trigger.split('+').forEach((part) => {
      ...
      switch (key) {
        ...
        case 'escape':
          ...
          break;
        case 'mouseleft':
          result.add('mouseleft');
          break;
        case 'mouseright':
          result.add('mouseright');
          break;
        case 'mousemiddle':
          result.add('mousemiddle');
          break;
        case 'mousebutton4':
        case 'mouse4':
        case 'button4':
          result.add('mousebutton4');
          break;
        case 'mousebutton5':
        case 'mouse5':
        case 'button5':
          result.add('mousebutton5');
          break;
        default:
          if (/^f\d+$/.test(key)) {
            result.add(key);
          } else if (/^[0-9]$/.test(key)) {
            result.add(`digit${key}`);

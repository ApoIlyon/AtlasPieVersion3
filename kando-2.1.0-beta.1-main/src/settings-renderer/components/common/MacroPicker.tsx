...
export default function MacroPicker(props: Props) {
  ...
  // Record mouse button presses while recording the macro.
  React.useEffect(() => {
    if (!recording) {
      return;
    }
...
/**
 * This method appends the button of the given MouseEvent to the input field.
 *
 * @param event The MouseEvent to record.
 * @param input The textarea element to update.
 */
function recordMouse(event: MouseEvent, input: HTMLTextAreaElement) {
  const macro = JSON5.parse('[' + input.value + ']' || '[]');
  ...
  macro.push({
    ...
    key: buttonMap.get(event.button) || `MouseButton${event.button}`,
    delay: 10,
  });

  input.value = convertToString(macro);
}

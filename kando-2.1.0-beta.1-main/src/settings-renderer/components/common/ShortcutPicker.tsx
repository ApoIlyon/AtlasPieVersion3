...
export default function ShortcutPicker(props: Props) {
  ...
  // While recording we also listen for mouse button presses globally. This allows
  // shortcuts to include mouse buttons such as the side buttons of modern mice.
  React.useEffect(() => {
    if (!recording) {
      return;
    }
    ...
    const onMouseDown = (event: MouseEvent) => {
      ...
      const isComplete = impl.recordMouse(event, input);
      if (isComplete) {
        setRecording(false);
        setShortcut(input.value);
        input.blur();
      }
...
class KeyNameImpl {
  ...
  /**
   * Appends a mouse button to the input field. Mouse buttons are detected via a global
   * listener in the ShortcutPicker component. The method returns true to indicate that
   * the shortcut is complete after the mouse button was added.
   *
   * @param event The mouse event to process.
   * @param input The input element to update.
   * @returns True if the shortcut is complete, false otherwise.
   */
  public recordMouse(event: MouseEvent, input: HTMLInputElement) {
    const parts = input.value.split('+').filter((part) => part !== '');
    ...
    const key = buttonMap.get(event.button) || `MouseButton${event.button}`;
  ...
  public isValidKey(key: string): boolean {
    const isKey =
      /^([0-9A-Z)!@#$%^&*(:;+=<,_\->.?/~`{\][|\\}"']|F1*[1-9]|F10|F2[0-4]|Plus|Space|Tab|Capslock|Numlock|Scrolllock|Backspace|Delete|Insert|Return|Enter|Up|Down|Left|Right|Home|End|PageUp|PageDown|Escape|Esc|VolumeUp|VolumeDown|VolumeMute|MediaNextTrack|MediaPreviousTrack|MediaStop|MediaPlayPause|PrintScreen|MouseRight|MouseMiddle|MouseButton4|MouseButton5|num(?:[0-9]|dec|add|sub|mult|div))$/;
...
class KeyCodeImpl {
  ...
  /**
   * Appends a mouse button to the input field. Returns true once the shortcut is complete
   * after adding the mouse button.
   *
   * @param event The mouse event to process.
   * @param input The input element to update.
   * @returns True if the shortcut is complete, false otherwise.
  ...
  public recordMouse(event: MouseEvent, input: HTMLInputElement) {
    ...
    const code = buttonMap.get(event.button) || `MouseButton${event.button}`;
   ...
   */
  public isValidKey(key: string): boolean {
    return isKnownKeyCode(key) && !this.isValidModifier(key) && key !== 'MouseLeft';
  }
}

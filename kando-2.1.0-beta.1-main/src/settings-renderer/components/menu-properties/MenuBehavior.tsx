...
export default function MenuBehavior() {
  ...
  return (
    <>
      ...
      />
      <Checkbox
        label={i18next.t('settings.close-on-trigger-release')}
        info={i18next.t('settings.close-on-trigger-release-info')}
        initialValue={menus[selectedMenu].closeOnTriggerRelease}
        onChange={(closeOnTriggerRelease) => {
          editMenu(selectedMenu, (menu) => {
            menu.closeOnTriggerRelease = closeOnTriggerRelease;
            return menu;
          });
        }}
      />

...
export class MenuWindow extends BrowserWindow {
  ...
  public async showMenu(
    ...
    if (this.isVisible()) {
      ...
      if (lastTrigger && request.trigger === lastTrigger) {
        if (this.lastMenu.closeOnTriggerRelease) {
          return;
        }
    ...
    if (!this.kando.allShortcutsInhibited() && sameShortcutBehavior === 'nothing') {
      ...
      await this.kando.getBackend().inhibitShortcuts([shortcut]);
    }
    ...
    this.webContents.send(
      ...
      this.lastMenu.root,
      {
        ...
        trigger: this.lastMenu.closeOnTriggerRelease
          ? useID
            ? this.lastMenu.shortcut
            : request.trigger
          : undefined,
        systemIconsChanged,
      },
  ...
  public show() {
    ...
    // Make sure the window captures mouse events again when it is shown.
  ...
  private async hideWindow() {
    ...
    // Let pointer events pass through while the menu fades out.

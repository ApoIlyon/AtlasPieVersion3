...
export class KandoApp {
  ...
  public async init() {
    this.backend.on('shortcutPressed', async (trigger) => {
      await this.showMenu({ trigger });
  ...
  /**
   * Returns the accelerator string of the shortcut that opened the last menu.
   *
   * @returns The accelerator string or undefined if no menu was shown yet.
   */
  public getLastTrigger(): string | undefined {
    return this.menuWindow?.lastMenu?.shortcut;
  }
  ...
  private updateTrayMenu(flavorChanged = false) {
    ...
    for (const menu of this.menuSettings.get('menus')) {
      ...
      template.push({
        ...
        click: async () => {
          await this.showMenu({ name: menu.root.name });
    ...
    if (this.inhibitAllShortcuts) {
      template.push({
        ...
        click: async () => {
          ...
          await this.backend.inhibitShortcuts([]);
          this.updateTrayMenu();
        },
      });
    } else {
      template.push({
        ...
        click: async () => {
          ...
          await this.backend.inhibitAllShortcuts();
          this.updateTrayMenu();
        },
      });
    }

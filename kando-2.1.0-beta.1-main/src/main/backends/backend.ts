  ...
  protected async bindShortcutsImpl(
    ...
    for (const shortcut of shortcutsToBind) {
      try {
        ...
        if (shortcut.toLowerCase().includes('mouse')) {
          console.warn(
            `Skipping unsupported mouse shortcut "${shortcut}": ${String(error)}`
          );
        } else {
          console.error(`Failed to register shortcut "${shortcut}": ${String(error)}`);
        }
      }
    }
  }

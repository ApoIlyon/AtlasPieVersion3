...
export type Native = {
  ...
  /**
   * Starts a global mouse hook which invokes the given callback for every mouse button
   * press together with the current modifier state.
   *
   * @param cb The callback to invoke for mouse events.
   */
  startMouseHook(
    cb: (event: {
      button: number;
  ...
  /** Stops the mouse hook installed by startMouseHook. */
  stopMouseHook(): void;
};

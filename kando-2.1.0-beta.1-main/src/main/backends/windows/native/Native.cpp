...
static std::thread               gMouseThread;
static std::atomic<bool>         gMouseRunning{false};
static Napi::ThreadSafeFunction  gMouseCallback;
static DWORD                     gMouseThreadId = 0;

LRESULT CALLBACK LowLevelMouseProc(int nCode, WPARAM wParam, LPARAM lParam) {
  if (nCode == HC_ACTION && gMouseCallback) {
    int            button = -1;
    ...
    switch (wParam) {
      ...
      case WM_MBUTTONDOWN:
        ...
        break;
      case WM_XBUTTONDOWN: {
        int xbtn = HIWORD(mouse->mouseData);
        if (xbtn == XBUTTON1) {
          button = 3;
    ...
    if (button != -1) {
      ...
      napi_status status = gMouseCallback.NonBlockingCall(
          data, [](Napi::Env env, Napi::Function cb, MouseHookData* data) {
            Napi::Object obj = Napi::Object::New(env);
            obj.Set("button", data->button);
            obj.Set("ctrl", data->ctrl);
            obj.Set("alt", data->alt);
            obj.Set("shift", data->shift);
            obj.Set("meta", data->meta);
            cb.Call({obj});
            delete data;
          });
...
void Native::startMouseHook(const Napi::CallbackInfo& info) {
  ...
  gMouseRunning = true;
  gMouseThread  = std::thread([]() {
    gMouseThreadId = GetCurrentThreadId();
    gMouseHook     = SetWindowsHookEx(WH_MOUSE_LL, LowLevelMouseProc, nullptr, 0);
    MSG msg;
    while (gMouseRunning.load() && GetMessage(&msg, nullptr, 0, 0)) {
    }
    if (gMouseHook) {
...
void Native::stopMouseHook(const Napi::CallbackInfo& info) {
  ...
  gMouseRunning = false;
  PostThreadMessage(gMouseThreadId, WM_QUIT, 0, 0);
  if (gMouseThread.joinable()) {
    gMouseThread.join();
  }

  if (gMouseCallback) {
    gMouseCallback.Release();

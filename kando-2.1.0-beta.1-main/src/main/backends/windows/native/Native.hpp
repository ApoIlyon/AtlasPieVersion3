...
class Native : public Napi::Addon<Native> {
 ...
 private:
  ...
  /**
   * Starts a global mouse hook and forwards button presses to JavaScript.
   *
   * @param info Expects a single callback function which will be invoked with
   *             information about the pressed mouse button and the state of
   *             modifier keys.
   */
  void startMouseHook(const Napi::CallbackInfo& info);

  /**
   * Stops the previously installed global mouse hook.
   *
   * @param info Expects no arguments.
   */

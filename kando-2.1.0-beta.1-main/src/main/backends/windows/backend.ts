...
export class WindowsBackend extends Backend {
  ...
  /** Mouse shortcuts bound through the native mouse hook. */
  private mouseShortcuts = new Set<string>();

  /** Indicates whether the native mouse hook is active. */
  ...
  /**
   * Binds global shortcuts. Keyboard-only shortcuts are registered via Electron's
   * globalShortcut API while shortcuts containing mouse buttons are handled through a
   * native hook.
   */
  protected override async bindShortcutsImpl(
    ...
    const isMouse = (s: string) => s.toLowerCase().includes('mouse');

    const keyboardNew = shortcuts.filter((s) => !isMouse(s));
    const keyboardOld = previouslyBound.filter((s) => !isMouse(s));
    await super.bindShortcutsImpl(keyboardNew, keyboardOld);

    this.mouseShortcuts = new Set(shortcuts.filter(isMouse));

    if (this.mouseShortcuts.size > 0 && !this.mouseHookStarted) {
      if (
        typeof native.startMouseHook === 'function' &&
        typeof native.stopMouseHook === 'function'
      ) {
        native.startMouseHook((event) => this.onMouseHook(event));
        this.mouseHookStarted = true;
      } else {
        console.warn(
          'Native mouse hook not available; rebuild the native module to enable mouse shortcuts'
        );
        this.mouseShortcuts.clear();
      }
    } else if (this.mouseShortcuts.size === 0 && this.mouseHookStarted) {
      if (typeof native.stopMouseHook === 'function') {
        native.stopMouseHook();
      }
  ...
  /**
   * Keeps mouse shortcuts bound even when they are temporarily inhibited. This avoids
   * repeatedly stopping and starting the native mouse hook which could otherwise miss
   * events when the menu is opened and closed in quick succession.
   */
  protected override async inhibitShortcutsImpl(
    shortcuts: string[],
    ...
    const boundShortcuts = this.getBoundShortcuts().filter(
      (s) => !previouslyInhibited.includes(s) || isMouse(s)
    );
    const shortcutsToBind = this.getBoundShortcuts().filter(
      (s) => !shortcuts.includes(s) || isMouse(s)
    );

    await this.bindShortcutsImpl(shortcutsToBind, boundShortcuts);
  }

  /** Handles mouse events coming from the native hook. */
  private onMouseHook(event: {
    button: number;
    ctrl: boolean;
    alt: boolean;
    shift: boolean;
    ...
    shortcut += buttonNames[event.button] ?? '';

    if (
      this.mouseShortcuts.has(shortcut) &&
      !this.getInhibitedShortcuts().includes(shortcut)
    ) {

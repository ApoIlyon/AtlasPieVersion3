...
export class HotkeyItemAction implements ItemAction {
  ...
  async execute(item: DeepReadonly<MenuItem>, app: KandoApp) {
    return new Promise<void>((resolve, reject) => {
      ...
      const trigger = app.getLastTrigger();
      const triggerKeys = new Set(
        trigger ? trigger.split('+').map((k) => k.toLowerCase()) : []
      );
      const keyboardKeys = keyNames.filter(
        (key) => !key.toLowerCase().startsWith('mouse')
      );

      // Ignore keys which are already held by the user to trigger the menu.
      const keysToPress = keyboardKeys.filter(
        (key) => !triggerKeys.has(key.toLowerCase())
      );

      // We simulate the key press by first pressing all keys and then releasing
      // them again in reverse order. We add a small delay between the key presses
      // to make sure that the keys are pressed in the correct order.
      const keys: KeySequence = [];

      // First press all required keys.
      for (const key of keysToPress) {
        keys.push({ name: key, down: true, delay: 10 });
      }

      // Then release all keys in reverse order.
      for (const key of [...keysToPress].reverse()) {

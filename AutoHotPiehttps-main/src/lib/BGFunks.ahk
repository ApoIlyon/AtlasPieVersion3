                ...
                for ahkHandleIndex, ahkHandle in profile.ahkHandles
                        {
                                ...
                                {
                                        hoverContext := MouseHover_GetContextForHandle(fullAHKHandle)
                                        if (hoverContext)
                                                Hotkey, If, % hoverContext
                                        else
		...
		if (SubStr(activeProfileString, -3) = "Func")       ;custom context
			...
			return
                ...
                Else                                                ;app specific context
                        ...
                        {
                                if (ActiveProfile.hoverActivation)
                                {
                                        hoverContext := MouseHover_GetContextForHandle(ahkHandle)
                                        if (hoverContext)
                                                Hotkey, If, % hoverContext
                                        else
...
ensurePieTargetWindowActive(windowID)
{
	if (!windowID)
		return false
	...
	WinWaitActive, ahk_id %windowID%, , 0.1
	if (!WinActive("ahk_id " windowID))
		DllCall("SetForegroundWindow", "ptr", windowID+0)
	return WinActive("ahk_id " windowID)
}

runPieFunction(functionObj)
	{
	global ActiveHoverWindowID
	targetWindow := ActiveHoverWindowID
	ActiveHoverWindowID := ""
	if(functionObj = false)
		return
	static lastPieFunctionRanTickCount := 0
	static lastPieFunctionRan = ""
	static lastPieFunctionWindow := ""
	...
	if (functionObj.function = "repeatLastFunction")
		...
		if (lastPieFunctionRan != "") && repeatTimeOut
			...
			if (targetWindow = "")
				targetWindow := lastPieFunctionWindow
			}
		else
			return
		}
	...
	if (targetWindow != "")
		ensurePieTargetWindowActive(targetWindow)
	...
	lastPieFunctionWindow := targetWindow
...
getActiveProfile()
	...
	;// check to see which hotkey condition matches
        ...
        hoverInfo := MouseHover_GetWindowInfo()
        if (IsObject(hoverInfo) && A_ThisHotkey != "")
        {
                for profileIndex, profile in Settings.appProfiles
                {
                        if (profile.enable == false)
                        ...
                        if (profile.ahkHandles[1] = "ahk_group regApps")
                                continue
                        if (SubStr(profile.ahkHandles[1], -3) = "Func")
                                continue
                        if (!profileHasHotkey(profile, A_ThisHotkey))
                                continue
                        if (MouseHover_ProfileMatches(profile, hoverInfo))
                                return profile
                }
        }
...
getProfileString(profile)
{
        if (profile.hoverActivation)
        {
                matchedHandle := MouseHover_ProfileMatches(profile)
                if (matchedHandle)
...
profileHasHotkey(profile, hotkey)
{
        if (!IsObject(profile))
                return false
        if (hotkey = "")
        ...
        for idx, pieKey in profile.pieKeys
        {
                if (pieKey.enable == false)
                        continue
                if (pieKey.hotkey = hotkey)
                        return true
        }

...
#Include %A_ScriptDir%\lib\MouseHoverActivation.ahk
...
pieLabel: ;Fixed hotkey overlap "r and ^r", refactor this
	...
	if (ActiveProfile.hoverActivation)
	{
		...
		if (IsObject(hoverInfo) && MouseHover_ProfileMatches(ActiveProfile, hoverInfo))
			ActiveHoverWindowID := hoverInfo.hwnd
	}
	...
	; tooltip % activeProfileString
        ...
        else if (ActiveProfile.hoverActivation)
        {
                hoverContext := MouseHover_GetContextForHandle(activeProfileString)
                if (hoverContext)
